...
async function PATCH_impl(
  ...
  try {
    ...
    const application = await prisma.application.findUnique({
      ...
      include: { jobPost: { select: { jobName: true, company_id: true } }, student: { include: { account: true } } },
    });
    ...
    const updatedApplication = await prisma.application.update({
      ...
      include: { applicationStatus: { select: { name: true } }, jobPost: { select: { jobName: true } }, student: { include: { account: true } } },
    });
    ...
    await prisma.notification.create({
      data: { account_id: updatedApplication.student.account.id, message, sender_id: Number(session.user.id) },
    });
